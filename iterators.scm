(library (iterators (0 0 0))
  ...
  (import (rnrs base (6))
		  ...
		  ;; (srfi 46) ;; Syntax rules using different ellipsis.
		  )
  (define (identity x) x)
  ...
  (define (iter->string iter)
	...
	(iter-for-each (lambda (c) (put-char c out)) iter)
  ...
  (define iter-range
	(case-lambda
	  ...
	  ((start end step)
	   ...
	   (iter-iota (+ 1 (div count step)) start step))))
  ...
  (define (iter-duplicate n iter)
	(if (<= n 0)
		...
		(iterate ((count 0) (item #f))
		  ...
		  (values item* (mod (+ count 1) n) item*))))
