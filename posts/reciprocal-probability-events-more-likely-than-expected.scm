...
{{post {version #1}}
 ...
 {published #"2025/01/07 15:57 +0000"}
 {uuid 9333a7f2-8695-4bb8-ab0a-c53189b5b6b7}
 {description
 ...
 {body
  {p
   ...
   I find this by noting that where the probability of the event is a half, the probability of it not happening is also a half.
   Therefore T, the number of times for this to be the case, satisfies
   #(math-block '(= (^ (- 1 (/ 1 137)) "T") (/ 1 2)))
  ...
  {p What I find more interesting about the estimation that it being wrong is that it's almost right.
   ...
   #(math-block '(= "T" (/ ("ln" 0.5) ("ln" (- 1 (^ "x" -1))))))
   What does this approach? Well for larger values of x, #(math '(- 1 (^ "x" -1))) approaches 1.
  ...
  {p
   ...
   #(math-block
     '(~~ ("f" "x")
          (lim (("h" inf)) (+ (* "x" ("f′" "h")) "h" (- #f (/ ("f" "h") ("f′" "h")))))))}
  #(let ((denom (lambda (x) `("ln" (- 1 (^ ,x -1))))))
     {p
      Some preliminary results for the final estimation:
      #(math-block
        `(stack
          (= ((/ "d" (* "d" "h")) (/ 1 ,(denom "h"))) (* (neg (^ "h" -2)) (^ (- 1 (^ "h" -1)) -1) (^ ,(denom "h") -2)))
          (= #f (* (^ (* (^ "h" 2) (- (^ "h" -1) 1)) -1) (^ ,(denom "h") -2)))
          (= #f (/ 1 (* (- "h" (^ "h" 2)) (^ ,(denom "h") 2))))
          (= ,(denom "h") (* 2 (+ (/ (neg (^ "h" -1)) (- 2 (^ "h" -1))) (* (/ 1 3) (^ (/ (neg (^ "h" -1)) (- 2 (^ "h" -1))) 3)) dots)))
          (= #f (* 2 (+ (/ (^ "h" -1) (- (^ "h" -1) 2)) (* (/ 1 3) (^ (/ (^ "h" -1) (- (^ "h" -1) 2)) 3)) dots)))
          (= #f (+ (/ 2 (- 1 (* 2 "h"))) (* (/ 2 3) (/ 1 (^ (- 1 (* 2 "h")) 3))) dots))
          (= (lim (("h" inf)) ((/ "d" (* "d" "h")) (/ 1 ,(denom "h"))))
             (lim (("h" inf)) (* (^ (- "h" (^ "h" 2)) -1) (^ (+ (/ 2 (- 1 (* 2 "h"))) (* (/ 2 3) (/ 1 (^ (- 1 (* 2 "h")) 3))) dots) -2))))
          ,@(math-rewrite
             '((= #f (lim (("h" inf)) (^ (* (tag h-h^2 (- "h" (^ "h" 2))) (^ (+ (/ 2 (- 1 (* 2 "h"))) (* (/ 2 3) (/ 1 (^ (- 1 (* 2 "h")) 3))) dots) 2)) -1)))
               (#f (h-h^2 neg (^ "h" 2)))))
          (= #f (lim (("h" inf)) (neg (^ (/ (* 2 "h") (- 1 (* 2 "h"))) -2))))
          (= #f -1)))
      ...
      #(math-block
        `(stack
          ...
          (= #f (+ (neg "x")
                   (lim (("h" inf))
                        (+ "h" (/ (- (* 2 (^ "h" 2)) (* 2 "h")) (- 1 (* 2 "h"))) (* (/ 2 3) (/ (- (^ "h" 2) "h") (^ (- 1 (* 2 "h")) 3))) dots))))
          (= #f (+ (neg "x") (lim (("h" inf)) (+ "h" (/ (- (* 2 (^ "h" 2)) (* 2 "h")) (- 1 (* 2 "h")))))))
          (= #f (+ (neg "x") (lim (("h" inf)) (+ "h" (- #f "h") 0.5 (/ 0.5 (- 1 (* 2 "h")))))))
          (= #f (- 0.5 "x"))))
  ...
  {p
   ...
   #(math-block '(= "T" (/ ("ln" (- 1 "P")) ("ln" (- 1 (^ "x" -1)))) (~~ #f (dot (neg ("ln" (- 1 "P"))) (- "x" 0.5)))))
   ...
   The estimate is also inaccurate for choices of x close to 1, i.e.
   ...
   Luckily its also not very useful for those values as they are very likely after only a single repeat.} {p This is perhaps the finest example of the difference between the median and the mean.
